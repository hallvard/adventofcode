...
public class Part2 {
    ...
    private final static String[] DIGITS = {"zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};

    private boolean lookingAt(String s, int pos, String substring) {
        for (int i = 0; i < substring.length(); i++) {
            if (pos + i >= s.length()) {
                return false;
            }
            if (s.charAt(pos + i) != substring.charAt(i)) {
                return false;
            }
        }
        return true;
    ...
    private int digit(String s, int pos) {
        char c = s.charAt(pos);
        if (Character.isDigit(c)) {
            return c - '0';
        }
        for (int i = 0; i < DIGITS.length; i++) {
            if (lookingAt(s, pos, DIGITS[i])) {
                return i;
            }
        }
        return -1;
    ...
    private int findDigit(String s, int start, int d) {
        while (start >= 0 && start < s.length()) {
            int digit = digit(s, start);
            if (digit >= 0) {
                return digit;
            }
            start += d;
        }
        throw new IllegalArgumentException("No digit in " + s);
    }

    public int compute(String input) {
        return input.lines()
            .mapToInt(line -> findDigit(line, 0, 1) * 10 + findDigit(line, line.length() - 1, -1))
            .sum();
    }
