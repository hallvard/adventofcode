...
public class Part1 {
    ...
    protected Pair<String, Map<String, Pair<String, String>>> parse(String input) {
        String[] tokens = input.split("\\W+");
        Map<String, Pair<String, String>> nodes = new HashMap<>();
        for (int i = 1; i < tokens.length; i += 3) {
            nodes.put(tokens[i], new Pair<>(tokens[i + 1], tokens[i + 2]));
        }
        return new Pair<>(tokens[0], nodes);
    }

    protected String next(Pair<String,String> choice, char step) {
        return switch(step) {
            case 'L' -> choice.item1();
            case 'R' -> choice.item2();
            default -> throw new IllegalArgumentException("Illegal step: " + step);
        };
    }
    ...
    public long compute(String input) {
        ...
        while (! node.equals("ZZZ")) {
            node = next(graph.get(node), steps.charAt(stepCount % steps.length()));
            stepCount++;
        }
        return stepCount;
    }
