...
public class Part2 {
    ...
    private int numAt(String line, int pos) {
        // looking at digit?
        if (pos < 0 || pos >= line.length() || !Character.isDigit(line.charAt(pos))) {
            return -1;
        }
        // scan backward
        int numStart = pos;
        ...
        return Integer.parseInt(line.substring(numStart, numEnd));
    }
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            for (int pos = 0; pos < line.length(); pos++) {
                if (isSymbol(line.charAt(pos))) {
                    int numOver1 = -1, numOver2 = -1;
                    if (lineNum > 0) {
                        var lineOver = lines[lineNum - 1];
                        if (Character.isDigit(lineOver.charAt(pos))) {
                            numOver1 = numAt(lineOver, pos);
                        } else {
                            numOver1 = numAt(lineOver, pos - 1);
                            numOver2 = numAt(lineOver, pos + 1);
                        }
                    }
                    int numLeft = numAt(line, pos - 1), numRight = numAt(line, pos + 1);
                    int numUnder1 = -1, numUnder2 = -1;
                    if (lineNum + 1 < lines.length) {
                        var lineUnder = lines[lineNum + 1];
                        if (Character.isDigit(lineUnder.charAt(pos))) {
                            numUnder1 = numAt(lineUnder, pos);
                        } else {
                            numUnder1 = numAt(lineUnder, pos - 1);
                            numUnder2 = numAt(lineUnder, pos + 1);
                        }
                    }
                    int numCount = 0, product = 1;
                    if (numOver1 >= 0) {
                        numCount++;
                        product *= numOver1;
                    } 
    ...
    public static void main(String[] args) {
        System.out.println(new Part2().compute(INPUT.split("\\s+")));
    }
