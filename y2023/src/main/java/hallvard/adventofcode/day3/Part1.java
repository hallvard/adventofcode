...
public class Part1 {
    ...
    private boolean isSymbolAt(String line, int pos) {
        return pos >= 0 && pos < line.length() && isSymbol(line.charAt(pos));
    }
    ...
    public int compute(String[] lines) {
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            lineLoop: for (int pos = 0; pos < line.length(); pos++) {
                ...
                if (Character.isDigit(c)) {
                    ...
                    if (pos + 1 == line.length()) {
                        // end-of-line, so number complete
                        numEnd = pos + 1;
                    }
                } else if (numStart >= 0) {
                    // not digit, so number complete
                    numEnd = pos;
                }
                if (numEnd >= 0) {
                    // complete number, so check symbol above or below
                    for (int pos2 = numStart - 1; pos2 <= numEnd; pos2++) {
                        if ((lineNum > 0 && isSymbolAt(lines[lineNum - 1], pos2)) ||
                            (isSymbolAt(line, pos2)) ||
                            (lineNum + 1 < lines.length && isSymbolAt(lines[lineNum + 1], pos2))
                            )
                         {
                            var num = Integer.parseInt(line.substring(numStart, numEnd));
                            sum += num;
                            numStart = numEnd = -1;
                            continue lineLoop;
                        }
    ...
    public static void main(String[] args) {
        System.out.println(new Part1().compute(INPUT.split("\\s+")));
    }
