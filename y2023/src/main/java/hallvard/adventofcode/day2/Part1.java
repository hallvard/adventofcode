...
public class Part1 {
    ...
    public record GameColors(int red, int green, int blue) {
        GameColors max(Color color, int value) {
            return switch (color) {
                case red -> new GameColors(Math.max(red, value), green, blue);
                case green -> new GameColors(red, Math.max(green, value), blue);
                case blue -> new GameColors(red, green, Math.max(blue, value));
            };
        }
        boolean lessOrEqual(GameColors colors) {
            return this.red <= colors.red && this.green <= colors.green && this.blue <= colors.blue;
        }
    }

    protected record Game(int id, GameColors colors) {}

    private GameColors parseGameColors(String s) {
        var gameColors = new GameColors(0, 0, 0);
        for (String colorCount : s.split("[;,]\\s*")) {
            int pos = colorCount.indexOf(' ');
            int count = Integer.parseInt(colorCount.substring(0, pos));
            Color color = Color.valueOf(colorCount.substring(pos + 1));
            gameColors = gameColors.max(color, count);
        }
        return gameColors;
    }

    protected Game parseGame(String line) {
        int pos = line.indexOf(':');
        if (! line.substring(0, pos).startsWith("Game ")) {
            throw new IllegalArgumentException("Should start with \"Game \"");
        }
        int gameId = Integer.parseInt(line.substring(5, pos));
        var gameColors = parseGameColors(line.substring(pos + 1).trim());
        return new Game(gameId, gameColors);
    }

    public int compute(String s, GameColors gameColors) {
        return s.lines()
            .map(this::parseGame)
            .filter(game -> game.colors().lessOrEqual(gameColors))
            .mapToInt(Game::id)
            .sum();
    }

    public static void main(String[] args) {
        System.out.println(new Part1().compute(INPUT, new GameColors(12, 13, 14)));
    }
