...
public class Part1 {
    ...
    public int compute(String input) {
        Map<String, Collection<String>> constraints = new HashMap<>();
        String[] lines = input.split("\n");
        int lineNum = 0;
        while (lineNum < lines.length) {
            String next = lines[lineNum++];
            if (next.isBlank()) {
                break;
            }
            String[] parts = next.split("[|]");
            constraints.computeIfAbsent(parts[0], key -> new ArrayList<>()).add(parts[1]);
        }
        int sum = 0;
        tokens: while (lineNum < lines.length) {
            String next = lines[lineNum++];
            String[] pages = next.split("[,]");
            for (int i = 0; i < pages.length; i++) {
                if (! checkConstraints(pages, i, constraints)) {
                    sum += handlePages(pages, constraints, false);
                    continue tokens;
                }
            }
            sum += handlePages(pages, constraints, true);
        }
        return sum;
    }
    ...
    private boolean checkConstraints(String[] pages, int pos, Map<String, Collection<String>> constraints) {
        var after = constraints.get(pages[pos]);
        if (after != null) {
            for (int i = 0; i < pos; i++) {
                if (after.contains(pages[i])) {
                    return false;
                }
            }
        }
